...
- Settings (bot token, chat id, topics, routing rules) live in the panel on the **Ops notifications** page (`/{panel}/ops-notify` → Settings), stored encrypted in `ops_notify_settings`. Docs: https://github.com/spokospace/filament-ops-notify/tree/main/docs
...
- Check a setup with `php artisan ops-notify:test` (sends right away and prints Telegram's error), then `php artisan ops-notify:test --queue` and confirm the new row turns *Sent* on the Ops notifications page.
