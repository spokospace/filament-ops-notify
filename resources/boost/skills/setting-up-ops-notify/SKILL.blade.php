...
1. Create a bot with @BotFather (`/newbot`; the username must end in `bot`) and paste the token into **Ops notifications → Settings** in the panel. Never ask them to paste it into the chat, `.env` or a file.
...
Then send through the queue, which also tests the worker. It prints `Queued, ...`; the new row on the Ops notifications page must turn *Sent* within seconds:
...
| Rows stay *Queued* | No worker for the queue; see **Delivery** on the Ops notifications page |
